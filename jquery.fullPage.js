...
(function($) {
	$.fn.fullpage = function(options) {
		...
		*/
		...
		$('.section').each(function(index){
			...
			if( activeSlide.length &&  ($('.section.active').index('.section') != 0 || ($('.section.active').index('.section') == 0 && activeSlide.index() != 0))){
				var prevScrollingSpeed = options.scrollingSpeed;
				$.fn.fullpage.setScrollingSpeed (0);
				landscapeScroll($('.section.active').find('.slides'), activeSlide);
				$.fn.fullpage.setScrollingSpeed(prevScrollingSpeed);
			}
			...
			if(options.navigation){
                ...
                var thisIndex = nav.find('li').eq($('.section.active').index('.section'));
				thisIndex.find('a').addClass('active');

                if(options.showActiveTooltip) {
                    thisIndex.append('<div class="fullPage-tooltip ' + options.navigationPosition +'">' + options.navigationTooltips[0] + '</div>');
                }
			}
		...
		*/
		...
		//navigation action
		$(document).on('click.fullpage', '#fullPage-nav a', function(e){
			e.preventDefault();
		 ...
		 */
		$('.section').on('click.fullpage', '.controlArrow', function() {
		 ...
		 * Scrolling horizontally when clicking on the slider controls.
		 */
		$('.section').on('click.fullpage', '.toSlide', function(e) {
			e.preventDefault();
		...
		* Scrolls the slider to the given slide destination for the given section
		*/
		$(document).on('click.fullpage', '.fullPage-slidesNav a', function(e){
			e.preventDefault();
			var slides = $(this).closest('.section').find('.slides');
