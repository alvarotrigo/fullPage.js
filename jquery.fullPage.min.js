...
c.length?c.each(function(){y(a(this))}):y(a(this))});a.isFunction(b.afterRender)&&b.afterRender.call(this)}function N(){if(!b.autoScrolling){var c=a(window).scrollTop(),d=0,e=Math.abs(c-a(".fp-section").first().offset().top);a(".fp-section").each(function(b){var f=Math.abs(c-a(this).offset().top);f<e&&(d=b,e=f)});var f=a(".fp-section").eq(d);if(!f.hasClass("active")&&!p){E=!0;var g=a(".fp-section.active").index(".fp-section")+1,n=F(f),k=f.data("anchor");f.addClass("active").siblings().removeClass("active");
a.isFunction(b.onLeave)&&b.onLeave.call(this,g,f.index(".fp-section")+1,n);a.isFunction(b.afterLoad)&&b.afterLoad.call(this,k,f.index(".fp-section")+1);G(k);H(k,0);b.anchors.length&&!p&&(w=k,location.hash=k);clearTimeout(O);O=setTimeout(function(){E=!1},100)}b.fitSection&&(clearTimeout(P),P=setTimeout(function(){p||q(f)},1E3))}}function Q(c){return scrollable=c.find(".fp-slides").length?c.find(".fp-slide.active").find(".fp-scrollable"):c.find(".fp-scrollable")}function z(c,b){if("down"==c)var e="bottom",
